...
class GitHubAuthProvider(AuthProvider):
    ...
    def get_auth_error_message(self, scope: dict | None = None) -> str:
        ...
        if scope:
            # Extract host and scheme from the ASGI scope
...
def _build_callback_url(request: Request) -> str:
    ...
    if not callback_url.startswith(("http://", "https://")):
        # If url_for returns a relative path, build the full URL
        callback_url = f"{request.url.scheme}://{request.url.netloc}{callback_url}"
    return callback_url
