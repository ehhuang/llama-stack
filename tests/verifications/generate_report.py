...
def run_tests(provider):
    ...
    try:
        ...
        if temp_json_file.exists():
            ...
            # Save results to our own format
...
def generate_report(results_dict, output_file=None):
    ...
    # Add overall totals
    overall_rate = f"{(passed_tests / total_tests * 100):.1f}%" if total_tests > 0 else "N/A"
    report.append(f"| **Total** | **{overall_rate}** | **{passed_tests}** | **{total_tests}** |")
    report.append("\n")
    ...
    for provider in sorted(
        ...
        report.append(f"## {provider.capitalize()}\n")
...
def main():
    ...
    parser.add_argument("--cleanup", action="store_true", help="Clean up old test result files")
    ...
    if args.run_tests:
        ...
        for provider, result_file in provider_result_files.items():
            provider_results = parse_results(result_file)
            all_results.update(provider_results)
