...
class MetaReferenceFilesImpl(Files):
    def __init__(self, config: MetaReferenceFilesImplConfig) -> None:
        self.config = config
        self.sql_store = None
    ...
    async def shutdown(self) -> None:
        """Cleanup resources."""
        # SQL store doesn't require explicit closing in current implementation
        pass
    ...
    async def openai_upload_file(
        ...
        # Set expiration to 1 year from now (OpenAI default)
        expires_at = created_at + (365 * 24 * 60 * 60)
    ...
    # Placeholder implementations for llama-stack native APIs (not implemented)
    async def create_upload_session(self, bucket: str, key: str, mime_type: str, size: int) -> FileUploadResponse:
        raise NotImplementedError("llama-stack native APIs not implemented")

    async def upload_content_to_session(self, upload_id: str) -> FileResponse | None:
        raise NotImplementedError("llama-stack native APIs not implemented")

    async def get_upload_session_info(self, upload_id: str) -> FileUploadResponse:
        raise NotImplementedError("llama-stack native APIs not implemented")

    async def list_all_buckets(self, bucket: str) -> ListBucketResponse:
        raise NotImplementedError("llama-stack native APIs not implemented")

    async def list_files_in_bucket(self, bucket: str) -> ListFileResponse:
        raise NotImplementedError("llama-stack native APIs not implemented")

    async def get_file(self, bucket: str, key: str) -> FileResponse:
        raise NotImplementedError("llama-stack native APIs not implemented")
