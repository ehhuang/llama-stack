...
def test_response_file_search_with_filters(request, openai_client, model, provider, verification_config, tmp_path):
    if isinstance(openai_client, LlamaStackAsLibraryClient):
        pytest.skip("Responses API file search is not yet supported in library client.")
    ...
    # Test 1: Filter by region (eq filter)
    tools = [
        {
            "type": "file_search",
    ...
    # Test 2: Filter by category (eq filter)
    tools = [
        {
            "type": "file_search",
            "vector_store_ids": [vector_store.id],
    ...
    # Test 3: Date range filter (compound with gte and lte)
    tools = [
        {
            "type": "file_search",
            "vector_store_ids": [vector_store.id],
    ...
    # Test 4: Complex compound filter (region AND category)
    tools = [
        {
            "type": "file_search",
            "vector_store_ids": [vector_store.id],
    ...
    # Test 5: OR filter (marketing OR sales)
    tools = [
        {
            "type": "file_search",
            "vector_store_ids": [vector_store.id],
