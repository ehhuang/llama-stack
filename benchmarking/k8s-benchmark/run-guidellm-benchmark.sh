    ...
    spec:
      ...
      - name: guidellm-benchmark
        ...
        - |
          ...
          # Run GuideLL benchmark and save output
          export COLUMNS=200
          GUIDELLM__PREFERRED_ROUTE="chat_completions" uv run guidellm benchmark \\
            --target "$BASE_URL" \\
            --rate-type "$RATE_TYPE" \\
            --max-seconds $MAX_SECONDS \\
            --data "prompt_tokens=$PROMPT_TOKENS,output_tokens=$OUTPUT_TOKENS" \\
            --model "$INFERENCE_MODEL" \\
            2>&1 | tee /tmp/guidellm-benchmark-output.txt
        env:
...
echo "Cleaning up any existing GuideLL benchmark job..."
kubectl delete job $JOB_NAME 2>/dev/null || true

echo "Deploying GuideLL benchmark Job..."
kubectl apply -f "$TEMP_YAML"

echo "Waiting for job to start..."
kubectl wait --for=condition=Ready pod -l job-name=$JOB_NAME --timeout=120s

echo "Following GuideLL benchmark logs..."
kubectl logs -f job/$JOB_NAME

echo "Job completed. Checking final status..."
kubectl get job $JOB_NAME

# Copy benchmark output from pod to local file
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
OUTPUT_FILE="guidellm-benchmark-${TARGET}-${STACK_REPLICAS}-${VLLM_REPLICAS}-${TIMESTAMP}.txt"
POD_NAME=$(kubectl get pods -l job-name=$JOB_NAME -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")

if [[ -n "$POD_NAME" ]]; then
    echo "Copying benchmark output to $OUTPUT_FILE..."
    kubectl cp $POD_NAME:/tmp/guidellm-benchmark-output.txt ./$OUTPUT_FILE 2>/dev/null || {
        echo "Warning: Could not copy output file from pod. Getting logs instead..."
        kubectl logs $POD_NAME > ./$OUTPUT_FILE
    }
    echo "Benchmark output saved to: $OUTPUT_FILE"
else
    echo "Warning: Could not find pod to copy output file"
fi
