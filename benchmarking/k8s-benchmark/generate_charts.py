...
def extract_setup_name(filename: str) -> str:
    """Extract setup name from filename."""
    # Pattern: guidellm-benchmark-{setup}-{instance}-{run}-{timestamp}.txt
    match = re.search(r"guidellm-benchmark-([^-]+(?:-\d+)*)-(\d+)-(\d+)-\d+\.txt", filename)
    if match:
        setup = match.group(1)
        instance = match.group(2)
        run = match.group(3)
        return f"{setup}-{instance}-{run}"
    return os.path.basename(filename).replace("guidellm-benchmark-", "").replace(".txt", "")
...
def generate_charts(benchmark_dir: str = "k8s-benchmark/results"):
    ...
    # Group setups by replica number
    replica1_data = {}
    replica2_data = {}
    other_data = {}

    for setup_name, points in all_data.items():
        # Extract replica number from setup name
        # Expected formats: stack-X-Y-timestamp or vllm-X-Y-timestamp where Y is replica number
        parts = setup_name.split("-")
        if len(parts) >= 4:  # Need at least setup-instances-replicas-timestamp
            try:
                # For patterns like stack-2-1-20250911 or vllm-1-2-20250912
                # The replica number is the 3rd component (index 2)
