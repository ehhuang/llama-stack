...
jest.mock('@/components/chat-completions/format-tool-call');
...
import { formatToolCallToString as originalFormatToolCallToString } from '@/components/chat-completions/format-tool-call';
...
describe('ChatCompletionsTable', () => {
  ...
  beforeEach(() => {
    ...
    formatToolCallToString.mockImplementation((toolCall: any) => toolCall && typeof toolCall === 'object' && toolCall.name ? `[DefaultToolCall:${toolCall.name}]` : "[InvalidToolCall]");
  });
  ...
  test('click on a row navigates to the correct URL', () => {
    ...
    rerender(<ChatCompletionsTable {...defaultProps} completions={[mockCompletion]} />);
  ...
  describe('Loading State', () => {
    test('renders skeleton UI when isLoading is true', () => {
      const { container } = render(<ChatCompletionsTable {...defaultProps} isLoading={true} />);
      ...
      if (tableBody) {
        const bodySkeletons = tableBody.querySelectorAll(`td ${skeletonSelector}`);
      ...
      const allSkeletonsInTable = container.querySelectorAll(`table ${skeletonSelector}`);
  ...
  describe('Error State', () => {
    test('renders error message when error prop is provided', () => {
      ...
      render(<ChatCompletionsTable {...defaultProps} error={{ name: 'Error', message: errorMessage }} />);
    ...
    test('renders default error message when error.message is not available', () => {
      render(<ChatCompletionsTable {...defaultProps} error={{ name: 'Error', message: '' }} />); // Error with empty message
      expect(screen.getByText('Error fetching data: An unknown error occurred')).toBeInTheDocument();
    });
  ...
  describe('Empty State', () => {
    test('renders "No chat completions found." and no table when completions array is empty', () => {
      render(<ChatCompletionsTable completions={[]} isLoading={false} error={null} />);
      expect(screen.getByText('No chat completions found.')).toBeInTheDocument();
    ...
    test('renders table caption, headers, and completion data correctly', () => {
      const mockCompletions = [
        ...
        },
        {
          ...
          input_messages: [
            { role: 'user', content: 'Another input' },
          ],
        },
      ];

      render(
        <ChatCompletionsTable completions={mockCompletions} isLoading={false} error={null} />
      );

      // Table caption
      expect(screen.getByText('A list of your recent chat completions.')).toBeInTheDocument();
      ...
      expect(screen.getByText(new Date(1710000000 * 1000).toLocaleString())).toBeInTheDocument();
      ...
      expect(screen.getByText(new Date(1710001000 * 1000).toLocaleString())).toBeInTheDocument();
    });
  });
  ...
  describe('Text Truncation and Tool Call Formatting', () => {
    test('truncates long input and output text', () => {
      ...
      truncateText.mockImplementation((text: string | undefined, maxLength?: number) => {
      ...
      const longInput = 'This is a very long input message that should be truncated.';
      const longOutput = 'This is a very long output message that should also be truncated.';
      const mockCompletions = [
        {
          id: 'comp_trunc',
          object: 'chat.completion',
      ...
      const truncatedTexts = screen.getAllByText(longInput.slice(0, 10) + '...');
      ...
      truncatedTexts.forEach(textElement => expect(textElement).toBeInTheDocument());
    });

    test('formats tool call output using formatToolCallToString', () => {
      // Specific mock implementation for this test
      formatToolCallToString.mockImplementation((toolCall: any) => `[TOOL:${toolCall.name}]`);
