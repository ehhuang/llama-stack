...
async def test_authorized_fetch_with_where_sql_access_control(mock_get_auth_attributes):
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        mock_get_auth_attributes.return_value = {"roles": ["admin"], "teams": ["engineering"]}
        ...
        rows = await sqlstore.authorized_fetch_all("documents", where={"id": 1})
        assert len(rows) == 1
        ...
        rows = await sqlstore.authorized_fetch_all("documents", where={"id": 2})
        assert len(rows) == 1
        assert rows[0]["title"] == "User Document"

        # Test authorized_fetch_one
        row = await sqlstore.authorized_fetch_one("documents", where={"id": 1})
        assert row is None  # Access denied

        row = await sqlstore.authorized_fetch_one("documents", where={"id": 2})
        assert row is not None
