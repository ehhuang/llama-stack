...
@pytest.mark.asyncio
@patch("llama_stack.providers.utils.sqlstore.sqlalchemy_sqlstore.get_auth_attributes")
async def test_secure_fetch_with_where_sql_access_control(mock_get_auth_attributes):
    """Test that secure_fetch_all still works correctly with where_sql for access control"""
    with TemporaryDirectory() as tmp_dir:
        ...
        mock_get_auth_attributes.return_value = {"roles": ["admin"], "teams": ["security"]}

        # Insert documents with access attributes
        await sqlstore.secure_insert(
            "documents", {"id": 1, "title": "Admin Document", "content": "Secret admin content"}
        )

        # Change user attributes
        mock_get_auth_attributes.return_value = {"roles": ["user"], "teams": ["marketing"]}

        await sqlstore.secure_insert("documents", {"id": 2, "title": "User Document", "content": "Public user content"})

        # Test that access control works with where parameter
        mock_get_auth_attributes.return_value = {"roles": ["admin"], "teams": ["security"]}

        # Admin should see both documents
        rows = await sqlstore.secure_fetch_all("documents", where={"id": 1})
        assert len(rows) == 1
        ...
        rows = await sqlstore.secure_fetch_all("documents", where={"id": 2})
        assert len(rows) == 1
        assert rows[0]["title"] == "User Document"

        # Test secure_fetch_one
        row = await sqlstore.secure_fetch_one("documents", where={"id": 1})
        assert row is None  # Access denied

        row = await sqlstore.secure_fetch_one("documents", where={"id": 2})
        assert row is not None
