...
import { formatToolCallToString } from "@/components/chat-completions/format-tool-call";
...
export function ChatMessageItem({ message }: ChatMessageItemProps) {
  switch (message.role) {
    case "system":
      return <MessageBlock label="System" content={message.content} />;
    case "user":
      return <MessageBlock label="User" content={message.content} />;
    ...
    case "assistant":
      if (message.tool_calls && message.tool_calls.length > 0) {
        ...
        return <MessageBlock label="Assistant" content={message.content} />;
      }
    case "tool":
      const toolOutputContent = (
        <ToolCallBlock>{message.content}</ToolCallBlock>
      );
      return (
