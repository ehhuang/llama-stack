...
/**
 * Generic pagination hook that can be used for any paginated API endpoint
 * 
 * This hook provides:
 * - Cursor-based pagination with infinite scroll support
 * - Progressive loading based on viewport dimensions
 * - Loading states (initial load, loading more)
 * - Error handling with customizable messages
 * 
 * @example
 * const paginationHook = usePagination({
 *   fetchFunction: async (params) => await api.list(params),
...
export function usePagination<T>({
   ...
   */
  const fetchData = useCallback(async (after?: string, targetRows?: number) => {
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        data: isInitialLoad ? response.data : [...prev.data, ...response.data],
