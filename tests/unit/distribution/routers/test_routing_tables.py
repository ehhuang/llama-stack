...
async def test_models_use_provider_model_id_as_id_priority_over_model_id(cached_disk_dist_registry):
    """Test that use_provider_model_id_as_id parameter takes priority over model_id when both are provided."""
    table = ModelsRoutingTable({"test_provider": InferenceImpl()}, cached_disk_dist_registry, {})
    await table.initialize()

    # Register model with both use_provider_model_id_as_id and model_id (use_provider_model_id_as_id should win)
    await table.register_model(
        model_id="ignored-model-id",
        provider_model_id="actual-provider-model",
        provider_id="test_provider",
        use_provider_model_id_as_id=True,
    )

    # Verify provider_model_id is used as identifier, not model_id
    models = await table.list_models()
    assert len(models.data) == 1
    model = models.data[0]
    assert model.identifier == "actual-provider-model"
    ...
    # Test that model_id is ignored
    with pytest.raises(ValueError):
        await table.get_model("ignored-model-id")
