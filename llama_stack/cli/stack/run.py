...
class StackRun(Subcommand):
    ...
    def _run_server_directly(self, config_file: Path | None, args: argparse.Namespace) -> None:
        """Run the server using uvicorn CLI subprocess."""
        import sys
        ...
        if config.server.tls_keyfile and config.server.tls_certfile:
            uvicorn_cmd.extend(["--ssl-keyfile", config.server.tls_keyfile])
            uvicorn_cmd.extend(["--ssl-certfile", config.server.tls_certfile])
            if config.server.tls_cafile:
                uvicorn_cmd.extend(["--ssl-ca-certs", config.server.tls_cafile])
            logger.info("HTTPS enabled with certificates")

        env = os.environ.copy()
        env["LLAMA_STACK_CONFIG"] = str(config_file)
        if args.env:
            for env_var in args.env:
        ...
        try:
            result = subprocess.run(uvicorn_cmd, env=env)
