...
def github_oauth_app():
    ...
    auth_config = {
        ...
        "github_redirect_uri": "http://localhost:8321/auth/github/callback",
...
def test_github_callback_success(github_oauth_app):
    ...
    # We need to manually set up the app with state
    from llama_stack.distribution.server.auth_routes import create_github_auth_router
    ...
    assert data["user_info"]["username"] == "test-user"
    assert data["user_info"]["email"] == "test@example.com"
    assert data["user_info"]["organizations"] == ["test-org-1", "test-org-2"]
...
def test_github_auth_provider_authorization_url():
    ...
    provider = GitHubAuthProvider(config)
    url = provider.get_authorization_url("test_state_123")
...
async def test_github_auth_provider_complete_flow():
    ...
    with patch("httpx.AsyncClient", MockAsyncClient):
        result = await provider.complete_oauth_flow("test_code")

    assert "access_token" in result
    assert result["token_type"] == "Bearer"
    assert result["user_info"]["username"] == "test-user"
    assert result["user_info"]["organizations"] == ["test-org-1", "test-org-2"]

    # Verify the JWT token
    token = result["access_token"]
    decoded = jwt.decode(token, "test_jwt_secret", algorithms=["HS256"], audience="llama-stack")
    assert decoded["sub"] == "test-user"
    assert decoded["github_username"] == "test-user"
    assert decoded["github_orgs"] == ["test-org-1", "test-org-2"]
