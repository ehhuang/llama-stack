...
class GitHubTokenAuthProvider(AuthProvider):
    ...
    async def _get_github_user_info(self, access_token: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            user_task = client.get(f"{self.config.github_api_base_url}/user", headers=headers, timeout=10.0)
            orgs_task = client.get(f"{self.config.github_api_base_url}/user/orgs", headers=headers, timeout=10.0)
