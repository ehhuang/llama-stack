...
Configuration:
    ...
    - Test cases are defined in YAML files within `tests/verifications/openai/fixtures/test_cases/`.
...
# Load verification config globally for report generation
VERIFICATION_CONFIG: dict = {}
try:
    config_path = Path(__file__).parent / "config.yaml"
    if config_path.exists():
        with open(config_path, "r") as f:
            VERIFICATION_CONFIG = yaml.safe_load(f) or {}
    else:
        print(f"Warning: config.yaml not found at {config_path}. Display names may be missing.")
except Exception as e:
    print(f"ERROR loading config.yaml: {e}. Display names may be missing.")
...
def generate_report(
    ...
    for test_name in sorted_tests:
        ...
        example_base_test_name = test_name

    if example_base_test_name is None:
        # This should not happen if sorted_tests is not empty, but handle defensively
        example_base_test_name = "unknown_test"
        example_case_id = "unknown_case"
    ...
    for provider in sorted(
        ...
        # Add test command for reproducing results using the dynamically found example
        test_cmd_all = f"pytest tests/verifications/openai/test_chat_completion.py --provider={provider} -v"
        test_cmd_specific_case = f'pytest tests/verifications/openai/test_chat_completion.py --provider={provider} -k "{example_base_test_name} and {example_case_id}"'
        report.append(
            f"```bash\n# Run all tests for this provider:\n{test_cmd_all}\n\n# Example: Run only the '{example_case_id}' case of {example_base_test_name}:\n{test_cmd_specific_case}\n```\n"
        )
