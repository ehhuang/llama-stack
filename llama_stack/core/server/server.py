...
class StackApp(FastAPI):
    ...
    def construct_stack(self, config: StackRunConfig):
        self.stack = Stack(config)

        # This is called from a sync context, so we need to run the initialize() method in a thread pool
...
def main(args: argparse.Namespace | None = None):
    ...
    # Create the app using the new create_app function
    try:
        app = create_app(
    ...
    # Get the config for server startup (need to reload it)
